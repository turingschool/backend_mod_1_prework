...
does_not_belong = "harry Potter does not beong in Hogwarts!"
p does_not_belong
...
# For first_name: a name is a word, so it's be a string.
# For is_hungry: beeing hungry is either true or not true, so it's be a boolean.
# For number_of_pets: quantaties are numbers, and since
  # you can't have half a pet, no matter how small it is, it won't be a float, so
  # it's an integer.
